...
export async function GET(request) {
  try {
    ...
    // ── Image Enhancement ───────────────────────────────────────────────
    // Priority 1: Google Drive URL → convert to thumbnail API (works for public files)
    // Priority 2: Existing direct URL → keep as-is
    // Priority 3: No URL at all → Unsplash keyword fallback
    const enhancedProducts = products.map(p => {
      ...
      if (image_url && (
        ...
        if (driveId) {
          ...
          return p; // ✅ Drive image resolved — done
        }
      }

      // ── Priority 2: Non-Drive URL exists → leave it untouched ──────
      if (image_url && image_url.startsWith('http')) {
        return p;
      }

      // ── Priority 3: No URL → Unsplash keyword-based fallback ───────
